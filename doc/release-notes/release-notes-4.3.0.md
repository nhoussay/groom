GROOM Core version *4.3.0* is now available from:  <https://github.com/groom-project/groom/releases>
...
- The chainstate database (which is used for tracking UTXOs) has been changed from a per-transaction model to a per-output model ([See PR 1801](https://github.com/nhoussay/groom/pull/1801)). Advantages of this model are that it:
...
- LevelDB has been upgraded to version 1.22 ([See PR 1738](https://github.com/nhoussay/groom/pull/1738)). This version contains hardware acceleration for CRC on architectures supporting SSE 4.2. As a result, synchronization and block validation are now faster.
