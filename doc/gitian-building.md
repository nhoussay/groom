...
*Setup instructions for a gitian build of GROOM Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the GROOM
...
to the GROOM GitHub release page.
...
- [Building GROOM Core](#building-groom-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/groom-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/groom-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/groom-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/groom-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/groom-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/groom-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/groom-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@groom.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/groom-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/groom-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/groom-osx-*-build.assert
```
