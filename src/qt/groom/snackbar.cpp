...
#include "qt/groom/forms/ui_snackbar.h"
#include "qt/groom/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(groomGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &groomGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
