...
#include "qt/groom/splash.h"
#include "qt/groom/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("groom-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in groom.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in groom.qrc)
...
/** Class encapsulating GROOM Core startup and shutdown.
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. GROOM can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    /// 6. Determine availability of data and blocks directory and parse groom.conf
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("GROOM Core"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // groom: links repeatedly have their payment requests routed to this process:
